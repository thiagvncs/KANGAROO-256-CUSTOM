...
uint32_t Kangaroo::CheckHash(uint32_t h, uint32_t nbItem, HashTable* hT, FILE* f) {
    ...
    vector<Point> Sp;
    for (uint32_t i = 0; i < nbItem; i++) {
        Sp.push_back(types[i] == TAME ? Z : keyToSearch);
    }
    vector<Point> S = secp->AddDirect(Sp, P);
    for (uint32_t i = 0; i < nbItem; i++) {
        if (hT) e = hT->E[h].items[i];
        else e = items + i;
        ok = (S[i].x.bits64[0] == e->x.i64[0]) && (S[i].x.bits64[1] == e->x.i64[1]) &&
...
void Kangaroo::CheckPartition(int nbCore, std::string& partName) {
    ...
    InitRange();
    InitSearchKey();
    int nbThread = std::min((int)pow(2.0, (int)log2(nbCore)), MERGE_PART);
    ::printf("Thread: %d\nCheckingPart\n", nbThread);
    ...
    double O = (1.0 - (double)nbWrong / (double)nbDP) * 100.0;
    ::printf("[%.3f%% OK][%s]\n", O, GetTimeStr(t1 - t0).c_str());
    if (nbWrong > 0) {
        ::printf("DP: %" PRId64 "\n", nbDP);
        ::printf("Wrong DP: %" PRId64 "\n", nbWrong);
    }
}
...
void Kangaroo::CheckWorkFile(int nbCore, std::string& fileName) {
    ...
    for (int s = 0; s < HASH_SIZE; s += block) {
        ...
        JoinThreads(thHandles, nbThread);
        FreeHandles(thHandles, nbThread);
        for (int i = 0; i < nbThread; i++) nbWrong += params[i].hStop;
...
void Kangaroo::Check(std::vector<int> gpuId, std::vector<int> gridSize) {
    ...
    t0 = Timer::get_tick();
    for (int i = 0; i < nbKey; i++) pts1.push_back(secp->ComputePublicKey(&priv[i]));
    t1 = Timer::get_tick();
    ...
    if (useGpu) {
...
#ifdef USE_SYMMETRY
                if (cpuPy[i].ModPositiveK1()) cpuD[i].ModNegK1order();
                lastJump[i] = jmp;
#endif
                if (IsDP(&cpuPx[i])) {
                    bool found = false;
        ...
        for (int i = 0; i < nb; i++) {
            bool ok = gpuPx[i].IsEqual(&cpuPx[i]) && gpuPy[i].IsEqual(&cpuPy[i]) && gpuD[i].IsEqual(&cpuD[i]);
            if (!ok) {
                nbFault++;
                if (firstFault) {
