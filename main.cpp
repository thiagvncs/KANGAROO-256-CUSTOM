...
void printUsage() {
  ...
  printf(
      " -g g1x,g1y,g2x,g2y,...: Specify GPU(s) kernel gridsize, default is "
      "2*(MP),2*(Core/MP)\n");
  printf(
      " -d: Specify number of leading zeros for the DP method (default is "
      "auto)\n");
  printf(" -t nbThread: Secify number of thread\n");
  printf(
      " -w workfile: Specify file to save work into (current processed key "
      "only)\n");
  printf(
      " -i workfile: Specify file to load work from (current processed key "
      "only)\n");
...
static string outputFile = "PrivateKeyFOUND.txt";
...
#ifdef USE_SYMMETRY
  printf("\033[01;33m[+] Kangaroo v" RELEASE
         " (with symmetry [256 CUSTOM edition])\n");
#else
  printf("\033[01;33m[+] Kangaroo v" RELEASE " [256 CUSTOM edition]\n");
#endif
